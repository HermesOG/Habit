...
- `db/verify.sql` — какие из функций, нужных `api/`, есть в проде (`public.*`) и на стенде (`public.stg_*`) и кто вправе их исполнять. Ожидание для прода после переноса: все девять есть, `anon` не может, `service_role` может.
- `db/smoke_delete.sql` — сквозной тест `/delete` на одноразовом пользователе: чужой секрет отвергается, после удаления не остаётся ни строки. Для стенда: `node tools/stage-sql.mjs db/smoke_delete.sql`. Прогнан на стенде 2026-09-12: ok.
